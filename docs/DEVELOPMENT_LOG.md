...
#### M16 修复 2（2026-08-21）— 面板布局 + 暂停叠加图标重设计
- **面板布局修复**：
  - 问题：面板从 y=0 到 winH（全高），覆盖标题栏和底部控件
  - 修复：面板限制在 `y=32`（标题栏底部）到 `y=barY`（控件栏顶部）之间
  - 切换按钮保持全高（方便点击）
  - hit-test 逻辑全部改为基于 `panelTop..panelBot` 区域
- **暂停叠加图标重设计**：
  - 枚举 `PauseIcon { None, Play, Pause }` 指定显示哪种图标
  - `showPauseOverlay(icon)` 接受参数指定图标类型
  - 暂停时 → 显示大三角形 ▶（指向右），2s 后淡出
  - 鼠标移入视频区（暂停状态）→ 重新显示 ▶，淡出
  - 单击视频区 → 从播放进入暂停显示 ||，从暂停恢复播放显示 ▶
  - 播放中鼠标移动 → 不显示任何图标
- **提交**：`016b16b`

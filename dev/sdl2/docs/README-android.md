...
src/core/android/SDL\_android.c
...
(They should be separated by spaces after the "LOCAL\_SRC\_FILES := " declaration)
...
3. Edit "YOURPROJECT/app/jni/CMakeLists.txt" to include your project (it defaults to
   ...
   as targets in your project, so you should have "target\_link\_libraries(yourgame SDL2 SDL2main)"
   in your CMakeLists.txt file. Also be aware that you should use add\_library() instead of
   add\_executable() for the target containing your "main" function.
...
Conceptually changing your icon is just replacing the "ic\_launcher.png" files in
...
them using the standard functions in SDL\_rwops.h.

There are also a few Android specific functions that allow you to get other
useful paths for saving and loading data:

* SDL\_AndroidGetInternalStoragePath()
* SDL\_AndroidGetExternalStorageState()
* SDL\_AndroidGetExternalStoragePath()

See SDL\_system.h for more details on these functions.
...
If SDL\_HINT\_ANDROID\_BLOCK\_ON\_PAUSE hint is set (the default),
...
a specific message (SDL\_RENDER\_DEVICE\_RESET) and restore your textures
manually or quit the app.

You should not use the SDL renderer API while the app going in background:

* SDL\_APP\_WILLENTERBACKGROUND:
  ...
  (eg call SDL\_SetRenderTarget(renderer, NULL))

* SDL\_APP\_DIDENTERFOREGROUND:
  GL context is restored, and the SDL renderer API is available (unless you
  receive SDL\_RENDER\_DEVICE\_RESET).

# Mouse / Touch events

In some case, SDL generates synthetic mouse (resp. touch) events for touch
(resp. mouse) devices.
To enable/disable this behavior, see SDL\_hints.h:

* SDL\_HINT\_TOUCH\_MOUSE\_EVENTS
* SDL\_HINT\_MOUSE\_TOUCH\_EVENTS

# Misc

For some device, it appears to works better setting explicitly GL attributes
before creating a window:
SDL\_GL\_SetAttribute(SDL\_GL\_RED\_SIZE, 5);
SDL\_GL\_SetAttribute(SDL\_GL\_GREEN\_SIZE, 6);
SDL\_GL\_SetAttribute(SDL\_GL\_BLUE\_SIZE, 5);
...
Android\_JNI\_SetupThread() before doing anything else otherwise SDL will attach
your thread automatically anyway (when you make an SDL call), but it'll never
detach it.

If you ever want to use JNI in a native thread (created by "SDL\_CreateThread()"),
...
Setting the EGL\_SWAP\_BEHAVIOR attribute of the surface to EGL\_BUFFER\_PRESERVED
...
* Android OS can decide to terminate your application by calling onDestroy()
  (see Activity life cycle). Your application will receive a SDL\_QUIT event you
  can handle to save things and quit.

Don't call exit() as it stops the activity badly.

NB: "Back button" can be handled as a SDL\_KEYDOWN/UP events, with Keycode
SDLK\_AC\_BACK, for any purpose.
